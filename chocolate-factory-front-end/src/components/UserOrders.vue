            ...
            <div class="col">
              <div class="form-group">
                ...
                <input
                  ...
                  id="fabricName"
                  name="fabricName"
                  v-model="ordersForSearch.factoryName"
          ...
          <div class="row">
            ...
            <div class="col-3 d-flex flex-column">
              <button
                ...
                @click.prevent="cancelOrder(userPurchase)"
...
function getUserOrders() {
  axios
    .get(`http://localhost:8080/WebShopAppREST/rest/purchases/${user.value.id}`)
    .then((response) => {
...
function cancelOrder(order) {
  ...
  axios.patch(
    "http://localhost:8080/WebShopAppREST/rest/customerTypes/",
    order
  );
  axios
    .post(
      "http://localhost:8080/WebShopAppREST/rest/purchaseCancellationHistory/",
      user.value
    )
    .then((response) => {
...
function searchOrders() {
  userPurchases.value = originalUserPurchases.value.filter((purchase) => {
    const matchingFactoryName = ordersForSearch.value.factoryName
      ? purchase.factory.name
          .toLowerCase()
          .includes(ordersForSearch.value.factoryName.toLowerCase())
      : true;
...
function sortOrders(event) {
  ...
  switch (selectedOption) {
    case "Sort by factory name asc":
      userPurchases.value.sort((a, b) => {
        if (a.factory.name.toLowerCase() < b.factory.name.toLowerCase())
          return -1;
        if (a.factory.name.toLowerCase() > b.factory.name.toLowerCase())
          return 1;
        return 0;
