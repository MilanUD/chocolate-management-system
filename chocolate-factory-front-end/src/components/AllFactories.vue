            ...
            <div class="col">
              <div class="form-group">
                ...
                <input
                  ...
                  id="chocolateName"
                  name="chocolateName"
                  v-model="fabricForSearch.chocolateName"
            ...
            <div class="col">
              <div class="form-group">
                ...
                <input
                  ...
                  id="location"
                  name="location"
                  v-model="fabricForSearch.location"
            ...
            <div class="col">
              <div class="form-group">
                ...
                <input
                  ...
                  id="rating"
                  name="rating"
                  v-model="fabricForSearch.rating"
          ...
          <div class="card custom-card">
            <div class="card-body">
              ...
              >
                ...
                <p>
                  {{ factory.location.address.city }},
                  {{ factory.location.address.postalCode }}
                </p>
                <p class="text-muted">
                  Latitude: {{ factory.location.latitude }}, Longitude:
              ...
              <div class="d-flex align-items-center justify-content-center">
                <button
                  @click.prevent="
                    loadInformationAboutSpecificFactory(factory.id)
                  "
...
function searchFactories() {
  factories.value = allFactories.value.filter((factory) => {
    const matchFactoryName = fabricForSearch.value.name
      ? factory.name
          .toLowerCase()
          .includes(fabricForSearch.value.name.toLowerCase())
      : true;
    const matchChocolateName = fabricForSearch.value.chocolateName
      ? factory.chocolates.some((chocolate) =>
          chocolate.name
            .toLowerCase()
            .includes(fabricForSearch.value.chocolateName.toLowerCase())
        )
      : true;
    const matchLocation = fabricForSearch.value.location
      ? factory.location.address.city
          .toLowerCase()
          .includes(fabricForSearch.value.location.toLowerCase())
      : true;
    const matchRating = fabricForSearch.value.rating
      ? factory.rating == fabricForSearch.value.rating
      : true;
    return (
      matchFactoryName && matchChocolateName && matchLocation && matchRating
    );
  });
}
...
function loadFactories() {
  axios
    .get("http://localhost:8080/WebShopAppREST/rest/chocolateFactory/")
    .then((response) => {
...
function sortFactories(event) {
  ...
  switch (selectedOption) {
    ...
    case "Sort by location asc":
      factories.value.sort((a, b) => {
        if (
          a.location.address.city.toLowerCase() >
          b.location.address.city.toLowerCase()
        )
          return 1;
        if (
          a.location.address.city.toLowerCase() <
...
function filterFactories() {
  factories.value = allFactories.value.filter((factory) => {
    const matchingFactoryWithChocolates = factory.chocolates.some(
      (chocolate) => {
        const matchingFlavor =
          selectedFlavor.value === "Default" ||
          selectedFlavor.value === chocolate.flavor;
        const matchingType =
          selectedType.value === "Default" ||
          selectedType.value === chocolate.type;
        return matchingFlavor && matchingType;
      }
    );
    const matchingFactoryWithoutChocolates =
      selectedFlavor.value === "Default" && selectedType.value === "Default"
        ? true
        : false;
    return (
      (matchingFactoryWithChocolates || matchingFactoryWithoutChocolates) &&
      (selectedOption.value === "showAll" ||
        (selectedOption.value === "showOpen" && factory.isOpen))
    );
  });
}
</script>
