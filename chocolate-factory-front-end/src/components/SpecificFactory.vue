<template>
  <div class="specificFactory">
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-4 d-flex flex-column justify-content-start">
              <button
                v-if="
                  user.userType === 'Manager' && user.factoryId === factory.id
                "
                @click.prevent="GoToChocolateInsertionForm"
            ...
            <div class="col-4 d-flex justify-content-end">
              <div>
                <button
                  v-if="
                    doesUserHaveOrdersInSpecificFactory === true &&
                    hasCommented !== true
                  "
                  @click.prevent="GoToCommentWritingForm"
          ...
          <div class="row">
            ...
            <div class="col-6 d-flex flex-column justify-content-center">
              <i
                :class="
                  factory.isOpen ? 'bi bi-door-open' : 'bi bi-door-closed'
                "
              ></i>
        ...
        <div class="row rowStyle">
          <div class="col">
            <div class="d-flex justify-content-start" style="margin-left: 30%">
              <button
                @click.prevent="ShowAllChocolates"
                class="btn btn-primary"
              >
                Show All Chocolates
              </button>
          ...
          <div class="col">
            <div class="d-flex justify-content-end" style="margin-right: 30%">
              <button @click.prevent="ShowAllComments" class="btn btn-primary">
                Show All Comments
              </button>
            </div>
          </div>
    ...
    <div
      v-if="
        isShowAllCommentsButtonPressed &&
            ...
            <div class="card-body">
                ...
                <div class="col-md-3">
                  <button
                    ...
                    :disabled="
                      ...
                      comment.factoryId != user.factoryId
                    "
              ...
              <div class="row">
                ...
                <div class="col-md-1 d-flex flex-column">
                  ...
                  <i
                    v-if="comment.status === 'Accepted'"
                    class="bi bi-check-circle"
                  ></i>
                  <i
                    v-if="comment.status === 'Declined'"
                ...
                <div class="col-md-2 d-flex flex-column">
                  <button
                    v-if="
                      ...
                      user.factoryId === chocolate.fabricId
                    "
                    class="btn btn-primary"
                    @click.prevent="editChocolate(chocolate)"
                  >
                    Edit
                  </button>
                  <div
                    v-if="user.userType == 'Customer' && chocolate.isInStock"
                    class="input-group"
                  >
                    <button
                      class="btn btn-outline-secondary"
                      type="button"
                      @click="decreaseQuantity(chocolate.id)"
                    >
                      -
                    </button>
                    <input
                      @input="validateQuantity(chocolate)"
                      :max="chocolate.stockQuantity"
                      type="number"
                      class="form-control"
                      id="quantity-input"
                      v-model="quantityToBuy[chocolate.id]"
                    />
                    <button
                      class="btn btn-outline-secondary"
                      type="button"
                      @click="increaseQuantity(chocolate)"
                    >
                      +
                    </button>
                  </div>
                  <div
                    v-if="
                      user.userType == 'Worker' && user.factoryId === factory.id
                    "
                    class="input-group"
                  >
                    <button
                      class="btn btn-outline-secondary"
                      type="button"
                      @click="decreaseQuantityByWorker(chocolate)"
                    >
                      -
                    </button>
                    <input
                      @input="validateQuantityByWorker(chocolate)"
                      type="number"
                      class="form-control"
                      id="quantity-input"
                      v-model="quantityToChange[chocolate.id]"
                    />
                    <button
                      class="btn btn-outline-secondary"
                      type="button"
                      @click="increaseQuantityByWorker(chocolate.id)"
                    >
                      +
                    </button>
                  </div>
              ...
              <div class="row">
                ...
                <div class="col-md-2">
                  <button
                    ...
                    "
                    class="btn btn-primary"
                    @click.prevent="deleteChocolate(chocolate)"
                  >
                    Delete
                  </button>
                  <button
                  ...
                  <button
                    ...
                    "
                    class="btn btn-success mt-2"
                    @click="changeStockQuantityByWorker(chocolate)"
                  >
                    Change quantity
                  </button>
                </div>
...
function changeStockQuantityByWorker(chocolate) {
  ...
  axios
    .patch(
      `http://localhost:8080/WebShopAppREST/rest/chocolates/${chocolate.id}`,
      chocolate
    )
    .then(() => {
      loadChocolates();
...
function canUserCommentCheck() {
  ...
  if (user.value.id) {
    axios
      .get(
        `http://localhost:8080/WebShopAppREST/rest/purchases/exists/${user.value.id}/${factoryId}`
      )
      .then((response) => {
...
function hasAlreadyCommented() {
  ...
  axios
    .get(
      `http://localhost:8080/WebShopAppREST/rest/comments/${user.value.id}/${factoryId}`
    )
    .then((response) => {
...
function loadFactory() {
  ...
  axios
    .get(`http://localhost:8080/WebShopAppREST/rest/chocolateFactory/${id}`)
    .then((response) => {
...
function loadComments() {
  const id = route.params.id;
  axios
    .get(`http://localhost:8080/WebShopAppREST/rest/comments/${id}`)
    .then((response) => {
...
function loadChocolates() {
  const id = route.params.id;
  axios
    .get(`http://localhost:8080/WebShopAppREST/rest/chocolates/${id}`)
    .then((response) => {
...
function applyDiscounts() {
  ...
  if (customerType.value.type !== "Bronze") {
    chocolates.value.forEach(
      (choco) => (choco.price -= choco.price * customerType.value.discount)
    );
  }
}
...
function AcceptComment(comment) {
  ...
  const factoryId = route.params.id;
  axios
    .put("http://localhost:8080/WebShopAppREST/rest/comments/", comment)
    .then(() => {
      axios
        .patch(
          `http://localhost:8080/WebShopAppREST/rest/chocolateFactory/${factoryId}`
        )
        .then(() => {
          loadFactory();
        });
...
function decreaseQuantityByWorker(chocolate) {
  if (
    chocolate.stockQuantity + (quantityToChange.value[chocolate.id] - 1) >=
    0
  ) {
...
.custom-card {
  ...
  background-color: rgb(137, 99, 60);
  padding: 1rem;
  margin-bottom: 1rem;
  border-radius: 5px;
  color: white;
