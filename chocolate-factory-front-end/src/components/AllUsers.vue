      ...
      <div class="row align-items-center">
        <div class="col">
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="fabricName" name="fabricName" v-model="userForSearch.name" placeholder="Enter user's name">
          </div>
        </div>
        <div class="col">
          <div class="form-group">
            <label for="chocolateName">Last name:</label>
            <input type="text" class="form-control" id="chocolateName" name="chocolateName" v-model="userForSearch.lastName" placeholder="Enter user's last name here">
          </div>
        </div>
        <div class="col">
          <div class="form-group">
            <label for="location">Username:</label>
            <input type="text" class="form-control" id="location" name="location" v-model="userForSearch.username" placeholder="Enter username here">
          </div>
        </div>
...
<div class="row">
  <div class="col-md-4">
    <div class="form-group">
      ...
      <select v-model="selectedUserType" class="form-control" id="filter1" name="filter1" @change="filterUsers()">
  ...
  <div class="col-md-4">
    <div class="form-group">
      ...
      <select v-model="selectedCustomerType" class="form-control" id="filter2" name="filter2" @change="filterUsers()">
        <option value="Default">Default</option>
  ...
  <div class="col-md-4 d-flex justify-content-end">
    <div class="form-group">
      ...
      <select class="form-control" id="sort" name="sort" @change="sortUsers($event)">
        <option value="Default">Default</option>
            ...
            <div v-for="user in users" :key="user.Id" class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-3">
                          <p v-if="user.userType === 'Customer' && user.type">Points: {{ user.type.points }}</p>
                        </div>
                        <div class="col-3">
                        </div>
                        <div class="col-3">
                            <button :disabled="user.isBlocked" v-if="user.userType !== 'Admin'" @click="block(user.id)" class="btn btn-danger">Block</button>
                        </div>
                    </div>
                </div>
            </div>
...
import {ref, onMounted} from 'vue'
    ...
    function block(userId){
      axios.patch(`http://localhost:8080/WebShopAppREST/rest/users/block${userId}`).then(() => {
        loadUsers();
      })
    }
    ...
    function searchUsers(){
        ...
        const matchUserName = userForSearch.value.name ? user.name.toLowerCase().includes(userForSearch.value.name.toLowerCase()) : true;
        const matchLastName = userForSearch.value.lastName ? user.lastName.toLowerCase().includes(userForSearch.value.lastName.toLowerCase()) : true;
        const matchUsername = userForSearch.value.username ? user.username.toLowerCase().includes(userForSearch.value.username.toLowerCase()) : true;
    ...
    function filterUsers(){
      users.value = originalUsers.value.filter(u => {
        const matchingUserType = selectedUserType.value === 'Default' || selectedUserType.value === u.userType;
        const matchingCustomerType = selectedCustomerType.value === 'Default' || u.type && selectedCustomerType.value === u.type.type
